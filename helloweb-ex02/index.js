...
const application = express()
    ...
    .use(express.urlencoded({extends: true})) // application/x-www-form-urlencoded --> 이러한 형식으로 오는 폼 데이터를 파싱해서 넣어줌
    .use(express.json())                      // application/json
    // 3. view engine setup
    .set('views', path(__dirname, 'views'))
    .set('view engine', 'ejs')
    // 4. request router(인증처리에 사용이 용의한 필터같은 존제) , router는 express
    .all('*', function(req, res, nest){
        req.locals.req = req;
        req.locals.res = res;
        next();  
    })
